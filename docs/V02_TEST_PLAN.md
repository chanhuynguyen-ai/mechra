...
Candidate: **0.2.0-dev.4**. Record actual observations and attach the exported Task Pane log. No item below is automatically passed by the Python tests.

| Gate | Steps | Required observation | Status |
|---|---|---|---|
| Environment | Run `scripts/install.ps1 -PreviousProjectRoot C:\AI_project\Mechra-v0.2.0-dev.3` with SOLIDWORKS closed | PowerShell parser/ownership checks pass; old agent migration verified; Python/HTTP tests pass; C# build has no errors; pure C# runner passes; registration path/version verified | Pending |
| Load | Start SOLIDWORKS 2025 SP1.2 and enable Mechra | Task Pane welcome shows v0.2.0-dev.4; Check Part/New chat/Save log are visible; exported log identifies the expected DLL | Pending |
...
| Blank template | Open a blank Part with Design Binder and environment folders; Check Part | Create readiness reported; solid/surface counts 0; metadata preserved; no geometry created | Pending |
| Inspect only | Check Part with and without existing geometry; Save log | Part/selection/Undo remain unchanged; readiness reasons, feature types and loaded DLL appear in log | Pending |
...
| Preconditions | Read-only Part, open sketch edit, extra model features, multiple configurations/bodies | Refused before Apply is offered; rechecked at Apply if state changes; no geometry mutation | Pending |
