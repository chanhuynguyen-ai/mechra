...
$sources = @('src\SolidWorksAddin\Services\CadContracts.cs','src\SolidWorksAddin\Services\CadValidation.cs','src\SolidWorksAddin\Services\ModelRevision.cs','tests\CadValidationTests.cs') | ForEach-Object { Join-Path $root $_ }
...
& $output (Join-Path $root 'src\Shared\tests\verification-cases.json') (Join-Path $root 'src\Shared\tests\feature-scope-cases.json')
